...
exports.resetTestFileIndexes = resetTestFileIndexes;
...
function getNextTestFile(type) {
    ...
    switch (type) {
        case 'video':
            ...
            config.currentIndex.video = (config.currentIndex.video + 1) % config.videos.length;
            break;
        case 'audio':
            file = config.audio[config.currentIndex.audio];
            config.currentIndex.audio = (config.currentIndex.audio + 1) % config.audio.length;
            break;
        case 'caption':
            file = config.captions[config.currentIndex.caption];
            config.currentIndex.caption = (config.currentIndex.caption + 1) % config.captions.length;
            break;
    }
    return path_1.default.join(config.baseDir, file);
}
function resetTestFileIndexes() {
    exports.testMediaConfig.currentIndex = {
        video: 0,
        audio: 0,
        caption: 0
    };
